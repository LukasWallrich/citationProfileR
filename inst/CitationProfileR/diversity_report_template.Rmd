...
Second, we obtained locations, specifically in which country is the cited paper written, of all authors of each reference by using the OpenStreetMap (OSM) API and `countrycode` package. By this measure, the country that has largest count of authors is *some country*. This method is limited in that we have to rely on the OSM API and the `countrycode` package, and the information output by these two tool may be inaccurate.
