...
#' get_location(sample_data_frame, "affiliation.name")
...
# citation_data <- read_csv("inst/test-data/test_data_crossref_results.csv")

get_location <- function(citation_data, affiliation_col_name){
  ...
  for(entry in 1:seq_len(nrow(citation_data))){
            ...
            if(stringr::str_count(affiliation, ",") > 0){
              ...
              if(!is.null(OSM_last_phrase)){
                ...
                if(is.na(citation_data$country_code[entry])){
                  ...
                  citation_data$country_code[entry] <- citation_data$country_code[entry]
                }
