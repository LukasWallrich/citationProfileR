...
```{r}
genderizeCountries <- read_csv("../country_codes.csv", show_col_types = FALSE)$"Code"
```
...
guessGender <- function(nameVector,
  ...
  final_output <- responseDF %>%
    select("name", "gender", "country_id", "probability")
...
```{r}
g <- guessGender(c("Natalie", "Liam", "Eamon"), countryCode = "FR")
ncol(g)
g
```
```{r}
