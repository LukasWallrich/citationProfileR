...
only_author <- strsplit(only_author, split = ",")[[1]] #https://www.programmingr.com/tutorial/how-to-use-strsplit-in-r/ (list in list problem)
only_author <- only_author[1]
...
test_na_doi <-   if (!is.na(only_author) & !is.na(only_author_title)){
    ...
    rcrossref::cr_works(flq = c(query.author = only_author), limit = 1,sort='relevance', select = c('DOI', 'title', 'author', 'created', 'published-print', 'published-online', 'publisher-location'))
